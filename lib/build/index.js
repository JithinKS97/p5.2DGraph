...
var Graph2D = /** @class */ (function () {
    function Graph2D(config) {
        ...
        this.plotEquation = function (equation) {
            var leftMostX = _this.getX(_this.pos.x);
            var rightMostX = _this.getX(_this.pos.x + _this.w);
            var noOfSamples = 100;
            ...
            for (var i = 1; i < xCoords.length; i++) {
                stroke(255, 0, 0);
                strokeWeight(2);
                if (_this.getYPixel(yCoords[i - 1]) < _this.pos.y &&
                    _this.getYPixel(yCoords[i]) < _this.pos.y) {
                    continue;
                }
                if (_this.getYPixel(yCoords[i - 1]) > _this.pos.y + _this.h &&
                    _this.getYPixel(yCoords[i]) > _this.pos.y + _this.h) {
                    continue;
                }
